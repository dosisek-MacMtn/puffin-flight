...
### Mapping for Claude Code (local addition)

This repository vendors the skill; the mapping below is the only change to upstream `SKILL.md`. See `VENDOR.md`.

- **Task tool** is the `Agent` tool.
- **`research` agent** is `subagent_type: Explore` (read-only; it cannot write files, which preserves write isolation by construction).
- **`general` agent** is `subagent_type: general-purpose`.
- The parent is this session. It remains the only writer of `run-metadata.json`, `architecture.md`, `coverage-ledger.json`, and `findings.json`.
- Run the two validators with the repo's Node: `node .claude/skills/security-audit/validate-coverage-ledger.cjs <ledger>` and `node .claude/skills/security-audit/validate-findings.cjs <findings>`.
- Claude Code sessions do not provide the OS-enforced sandbox described below. Unless one is available, do not execute target-controlled code: record the lead as `needs_validation` with the missing sandbox capability, per the rules in that section.
